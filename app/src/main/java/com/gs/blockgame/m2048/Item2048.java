...
public class Item2048 extends View implements Cloneable{
	/**
	 * ��View�ϵ�����
	 */
	private int mNumber;
	...
	public void setNumber(int number)
	{
		mNumber = number;
		mNumberVal = mNumber + "";
		mPaint.setTextSize(30 * getResources().getDisplayMetrics().density);
		mBound = new Rect();
		mPaint.getTextBounds(mNumberVal, 0, mNumberVal.length(), mBound);
		invalidate();
	}

	/*private void numberSize(int len)
	{
		float size = 40;
		if (len > 3) {
			size = size * Math.pow(0.6d , len - 1);
		}
	}*/
